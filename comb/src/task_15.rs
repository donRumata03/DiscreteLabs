...
struct Combinations<'a> {
    descriptor: &'a CombinationsDescriptor,
    data: Vec<u64>,
}

#[derive(Clone, Debug)]
struct CombinationsDescriptor {
    n : usize,
    k : usize,
    sorted_alphabet: Vec<u64>,
    pre_counted_comb: Vec<Vec<u64>>
}

impl CombinationsDescriptor {
    fn new (n: usize, k : usize) -> CombinationsDescriptor {
       let mut res = CombinationsDescriptor{
            n,
            k,
            sorted_alphabet: (1_u64..=(n as u64)).collect(),
            pre_counted_comb: vec![vec![0; k + 1]; n + 1]
       };

       // Pre-count combs

       for nn in 0..=n {
            res.pre_counted_comb[nn][0] = 1;
       }

       for nn in 1..=n {
            for kk in 1..=k {
                res.pre_counted_comb[nn][kk] = 
                    res.pre_counted_comb[nn - 1][kk] 
                    + res.pre_counted_comb[nn - 1][kk - 1];
            }
       }
    ...
    fn get_comb_number(&self, n: usize, k: usize) -> u64 {
        self.pre_counted_comb[n][k]
    }
}

impl<'a> Combinations<'a> {
    pub fn new(descriptor: &'a CombinationsDescriptor) -> Self {
        Combinations { descriptor, data: Vec::with_capacity(descriptor.k) }
    }
}

impl CombinatorialObject<u64> for Combinations<'_> {
    fn is_self_contained(&self) -> bool {
        self.data.len() == self.descriptor.k
    }
    ...
    fn can_add(&self, element: &u64) -> bool {
        self.data.len() < self.descriptor.k
            && self.data
                .last()
                .iter()
                .all(|&last| last < element)
    }

    fn count_successors(&self) -> u64 {
        let numbers_left = self.descriptor.n - (*self.data.last().unwrap_or(&0) as usize);
        let positions_left = self.descriptor.k - self.data.len();

        self.descriptor.get_comb_number(numbers_left, positions_left)
    }
...
fn main() {
    ...
    let k = input.next::<usize>();
    let query_index = input.next::<u64>();


    let descriptor = CombinationsDescriptor::new(n, k);
    let mut pref = Combinations::new(&descriptor);


    let ans = combinatorial_object_by_number(query_index, &mut pref, &descriptor.sorted_alphabet);
