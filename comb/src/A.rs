...
#[derive(Clone, Debug)]
struct Sequence<'a> {
    descriptor: &'a SequenceDescriptor,
    data: Vec<u64>,
}
...
impl<'a> Sequence<'a> {
    pub fn new(descriptor: &'a SequenceDescriptor) -> Self {
        Sequence { descriptor, data: Vec::with_capacity(descriptor.len), /*sorted_alphabet: (1..=states).collect()*/ }
    }
}

impl CombinatorialObject<u64> for Sequence<'_> {
    fn is_self_contained(&self) -> bool {
    ...
    fn count_successors(&self) -> u64 {
        return (self.descriptor.states as u64)
            .pow((self.descriptor.len - self.data.len()) as u32);
    }
...
fn main() {
    ...
    let n = input.next();
    let descriptor = SequenceDescriptor { len: n, states: 2 };
    let mut pref = Sequence::new(&descriptor);
    ...
    for seq in ans.iter() {
        output.println(seq.data.iter().map(u64::to_string).collect::<Vec<_>>().join(""));
    }
}