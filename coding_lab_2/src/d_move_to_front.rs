...
fn move_to_front(input: &Vec<u8>) -> Vec<usize> {
	let mut queue = ('a' as u8..='z' as u8).into_iter().collect::<Vec<_>>();
	let mut res = Vec::new();

	for c in input.iter() {
		let index_of_c = queue.iter().position(|e| *e == *c);
		match index_of_c {
			None => {panic!()},
			Some(i) => {
				queue.remove(i);
