...
fn inverse_burrows(last_col: &Vec<u8>, result_index: usize) -> Vec<u8> {
	let start = 'a' as u8;
	let end = 'z' as u8;
	let range = start..=end;

	let mut next_position_by_element = vec![None; range.len()];
	let mut sorted = last_col.clone();
	sorted.sort();

	for (i, &c) in sorted.iter().enumerate() {
		let index = (c - start) as usize;
		let mut mut_ref = &mut next_position_by_element[index];
		match mut_ref {
			None => { *mut_ref = Some(i) },
			Some(_) => {}
		}
	...
	let mut inv = vec![0; last_col.len()];
	for (i, &c) in last_col.iter().enumerate() {
		inv[next_position_by_element[(c - start) as usize].unwrap()] = i;
		next_position_by_element[(c - start) as usize] = Some(next_position_by_element[(c - start) as usize].unwrap() + 1);
	}

	let mut res = Vec::new();
	let mut j = inv[result_index];

	for _ in 0..last_col.len() {
		res.push(last_col[j]);
		j = inv[j];
	}
...
fn main() {
	...
	// println!("{}", input_string.pop().unwrap());
	let mut data = unsafe { input_string.as_bytes_mut().to_vec() };
	let res = (0..data.len()).map(|index| inverse_burrows(&data, index)).min().unwrap();
