...
fn lzw_encode(input: &Vec<usize>) -> Vec<usize> {
	let mut dict = (0..26).into_iter().map(|c| vec![c]).collect::<Vec<_>>();
	let mut buffer = Vec::new();
	let mut res = Vec::new();

	for &c in input {
		let mut encoding_candidate = buffer.clone();
		encoding_candidate.push(c);

		match dict.iter().enumerate().find(|(_, v)| **v == encoding_candidate) {
			None => {
				// But &buff € dict!

				res.push(dict.iter().position(|v| *v == buffer).unwrap());
				dict.push(encoding_candidate);
				buffer.clear();
			},
			Some((_, _)) => {}
		}
...
fn main() {
	...
	let input_vector = input_string.as_bytes().iter().map(|&c| (c - ('a' as u8)) as usize).collect();

	let lzw_codes = lzw_encode(&input_vector);

	let ans_strings = lzw_codes.iter().map(|&int| int.to_string()).collect::<Vec<_>>();
	println!("{}", ans_strings.join(" "));
}
