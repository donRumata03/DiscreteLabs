...
fn main() {
	...
	for _ in 0..data.len() {
		rotations.push(data.clone());
		data.rotate_right(1);
	}
	rotations.sort();

	// println!("{:?}", rotations.iter().map(|s| String::from_utf8(s.clone()).unwrap()).collect::<Vec<_>>());

	println!("{}", String::from_utf8(rotations.iter().map(|s| *s.last().unwrap()).collect()).unwrap());
}
